...
uint32_t m_request_earliest(enum NRF_RADIO_PRIORITY priority)
{
    ...
    m_beacon.timeslot_request.params.earliest.hfclk       = NRF_RADIO_HFCLK_CFG_NO_GUARANTEE;
...
static nrf_radio_signal_callback_return_param_t * m_timeslot_callback(uint8_t signal_type)
{
	...
	switch (signal_type)
	{
		case NRF_RADIO_CALLBACK_SIGNAL_TYPE_START:
			// Start of the timeslot. The application now has exclusive access to the peripherals
			// for the full length of the timeslot.
			m_handle_start();
		...
		case NRF_RADIO_CALLBACK_SIGNAL_TYPE_RADIO:
			// Radio interrupt, for more information, see chapter 2.4GHz radio (RADIO0) in
			// the nRF52 Reference Manual
			if (NRF_RADIO->EVENTS_DISABLED == 1) {
				...
				if (mode == ADV_DONE) {
					...
					if (m_beacon.keep_running) {
						...
						// The current timeslot has ended. The SoftDevice is requested to schedule a new timeslot, after
						// which it can resume other activities.
						signal_callback_return_param.callback_action = NRF_RADIO_SIGNAL_CALLBACK_ACTION_REQUEST_AND_END;
					}
					else {
						// The current timeslot has ended. The SoftDevice can now resume other activities.
...
void app_beacon_sd_evt_signal_handler(uint32_t event)
{
    ...
    {		
        case NRF_EVT_RADIO_SESSION_IDLE:
			// The current timeslot session has no remaining scheduled timeslots.
        ...
        case NRF_EVT_RADIO_SESSION_CLOSED:
			// The timeslot session is closed and all acquired resources are released.
            break;
        case NRF_EVT_RADIO_BLOCKED:
			// The last requested timeslot could not be scheduled, due to a collision with already
			// scheduled activity or for other reasons.
        case NRF_EVT_RADIO_CANCELED: // Fall through
			// The scheduled timeslot was canceled due to overlapping activity of higher priority.
            if (m_beacon.keep_running)
            {
